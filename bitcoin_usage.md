...
7. **锁定**脚本中的CHECKSIG：该指令使用公钥来验证签名（签名是消费者使用自己的私钥对上一笔UTXO交易签名生成），签名算法是ECDSA。
具体过程是将上一笔UTXO所属交易部分数据进行哈希得到交易hash，然后使用公钥解密签名得到另一个哈希，对比两个哈希是否一致，一致就入栈1，否则入栈0。
