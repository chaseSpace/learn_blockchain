...
<!-- TOC -->
  ...
  * [数据层](#数据层)
    * [1. 账户原理](#1-账户原理)
    * [2. 交易原理](#2-交易原理)
    * [3. 区块从创建到入库](#3-区块从创建到入库)
    * [4. 小结](#4-小结)
  * [网络层](#网络层)
    * [1. Kad网络协议](#1-Kad网络协议)
    * [2. P2P节点通信](#2-P2P节点通信)
  * [共识层](#共识层)
    * [1. PoW算法和PoA算法](#1-PoW算法和PoA算法)
    * [2. PoS算法](#2-PoS算法)
    * [3. Casper算法](#3-Casper算法)
  * [激励层](#激励层)
  * [合约层](#合约层)
    * [1. 创建合约](#1-创建合约)
    * [2. 执行合约](#2-执行合约)
    * [3. 合约调用](#3-合约调用)
    * [4. Gas和Gas优化](#4-Gas和Gas优化)
  * [应用层](#应用层)
    * [1. 钱包](#1-钱包)
    * [2. DApp](#2-DApp)
  * [核心功能总览](#核心功能总览)
  ...
  * [开发环境搭建](#开发环境搭建)
    * [1. 安装geth](#1-安装geth)
    * [2. 连接主网](#2-连接主网)
    * [3. 搭建单节点私有链](#3-搭建单节点私有链)
  * [以太坊新技术](#以太坊新技术)
    * [1. 以太坊联盟链Quorum](#1-以太坊联盟链Quorum)
    * [2. 以太坊分片技术](#2-以太坊分片技术)
    * [3. 以太坊侧链技术](#3-以太坊侧链技术)
<!-- TOC -->
...
以太坊2.0选择的共识算法是Casper FFG，它采用的是LMD-GHOST分叉选择规则，LMD GHOST同时也是CBC选择的分叉选择规则。
...
合约创建的第一步是使用合约语言编写合约代码，目前用来编写以太坊合约的语言是Solidity、Vyper、Serpent和LLL，其中Solidity最为通用。编写完合约代码后，
...
作为该函数的唯一标识，解释器通过这4字节知道该调用哪个函数。然后通过`CALLDATALOAD`指令将前4字节入栈，再依次跟合约中的函数进行对比，匹配的函数被调用JUMPI指令跳入改代码继续执行； 而后面的字节就是该函数需要的参数，长度不定。
...
### 4. Gas和Gas优化
合约的部署和调用都会消耗一定的Gas数量，它作为矿工提供计算和存储资源的报酬，也是为了防止以太坊网络遭到DoS攻击。

#### 4.1 关于Gas
Gas译作汽油，形象地比作维持以太坊平台持续运行的燃料。Gas的出现是在用户发起交易时，用户需要为一笔**普通转账交易**支付21000 Gas。
同时，由于交易可以通过`Input Data`字段携带合约相关的字节码数据以达到部署合约或调用合约函数的目的，而合约字节码数据的计算、带宽和存储会消耗节点资源，
所以也需要用户根据其中的资源使用量进行付费。

#### 4.2 Gas计价规则
首先，Gas计费有几个基本原则：
- 先扣费，后执行：具体来说，就是根据合约编译后的一条条操作码进行先扣费后执行原则（判断粒度是操作码不是整个合约，因为部分操作的Gas消耗是无法提前得知的）。
- 提前中止不退费：执行过程中，可能出现用户预设的Gas数量不足以支付操作码执行所需的Gas费用，那么合约执行会中止，且已扣费的Gas不会退还。
- 释放资源可退费：具体来说，就是执行过程中，若某些操作码释放了存储资源，如重置某个状态变量或销毁合约，则会在当次调用结束后退还部分Gas，但不超过已使用Gas的一半。
...
在2021年8月5日以太坊完成伦敦升级后，改进的EIP-1559 用两个新参数（BaseFees和PriorityFees）替换了旧的`GasPrice`参数：
- `BaseFees`：在以太坊上发送交易或完成操作所需的最低Gas价格。基本费由协议动态调整，但记住一点，基本费是会被销毁的，即不会支付给矿工。
- `PriorityFees（Tip）`：优先级费用也叫做小费，用户为了让矿工优先处理自己的交易或者操作，支付给矿工的小费，由用户自行设置，默认情况下，预计这笔小费是2Gwei。
- `MaxFees`：=`BaseFees` + `PriorityFees`，相当于之前的`GasPrice`，但增加了一个**最高**的定语，之所以说是最高，是因为实际用到的小费可能不会达到预设的`PriorityFees`这么高，剩余部分会退还。

所以，更新后的总交易开销（transaction cost）= `MaxFees` * `GasUsed`。
>EIP-1559由以太坊核心开发者在2019年4月正式提出，目的是优化以太坊网络的Gas收费市场，能够一定程度上防止针对以太坊网络的垃圾交易攻击。
...
由上节中的`PriorityFee`可以知道，矿工对交易的打包（包含对附带的合约字节码执行）采用**高价拍卖原则**。

区块Gas限制也是定期通过矿工们投票调整的，系统将基于当时的网络需求来决定最终结果。
...
```
其中`1e-9`是把Gwei转Ether单位，$3829.61是交易发生时1ETH的美元价格。由从可知，创建一个合约的成本不可谓不高！

所以**优化合约代码以减少Gas费用**就成为了一个非常重要的主题。关于具体的优化措施，读者可以在熟悉solidity合约语言后再参阅由笔者编写的Gas优化笔记
[lesson18_savingGas.sol](./test_solidity/basic_exercises/lesson18_savingGas.sol)
和 [lesson19_savingGas2.sol](./test_solidity/basic_exercises/lesson19_savingGas2.sol) 。
...
该标准自2015年提出以来，逐步确定为下表中的11个标准接口。接口可以分为三类：常量、功能函数和事件：
...
其具体实现方式可以分为单一托管模式、联盟模式、SPV模式、驱动链模式和混合模式5类。
