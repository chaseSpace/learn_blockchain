...
- [背景](#背景)
    - [1.共识算法简史](#1-共识算法简史)
    - [2.早期数据库系统中的共识机制（分布式一致性方案）](#2-早期数据库系统中的共识机制（分布式一致性方案）)
    - [3.拜占庭将军问题](#3-拜占庭将军问题)
...
这两个方案都不考虑恶意节点，只考虑节点本身工作异常和不响应的情况。
...
PreCommit的节点在超时后会回滚事务）。因此，无论两阶段提交或三阶段提交，都不能作为一个健壮的分布式共识算法，只能在通常情况下保障分布式系统的一致性。
...
1. 51%算力。也就是说，除非攻击者拥有51%及以上的算力资源时，才能对主链发起有效攻击。虽然达到这个条件需要一定的成本，但并不是说不可能，所以这也可以说是概率问题。
...
>之所以叫传统PoS，是因为其本身具有一些缺陷，导致实际应用时会进行优化，或者说变体。

#### 2.1 基本概念
- 验证者：在PoS中，参与共识的节点被称为验证者节点（Validator）。任何拥有虚拟币的节点都可以通过发送特殊交易的方式，将部分虚拟货币转为股份，以成为验证者。完整的验证者节点集合由区块链系统维护。
- 币龄：为了描述持币情况，PoS共识算法引入了【币龄（）Coinage】概念，表示持有部分虚拟货币的时长。当节点将虚拟币作为股份投入后，这部分虚拟币就开始积累币龄，
...
**冷启动问题**  
由于PoS共识算法中币龄越大的节点优先获得记账权，所以参与节点都希望囤积虚拟币，很少交易。这就造成了纯粹了PoS公链无法冷启动。因此，实际运行时，
一般采用PoW算法启动区块链系统，再切换到PoW+PoS方式，最后切换到完全的PoS。目前，**以太坊正处于由PoW向PoS过渡的阶段**。
...
DPoS 最初由 Bitshares、Steemit 以及 EOS 的创办人 Dan Larimer 提出，他在区块链项目 Bitshares 中实现了 DPoS 共识机制。
>DPoS的设计者认为，从规模化角度看，PoW和PoS算法都有走向委托制的倾向，存在中心化风险。因此，不如在一开始就设计好如何进行权益分配与权力制约，有利于系统更好的运行，
>从而闭麦被动演化导致不可预期的结果。

DPoS是目前看到的最快、最高效、也最去中心化和最灵活的共识算法。委托权益证明（Delegated Proof of Stake, DPoS）利用权益人投票的权利来公平民主的解决共识问题。
...
然后见证人轮流生成区块。在一个生成周期结束后，再重新进行见证人选举。在使用DPoS算法的不同项目中，会对具体流程做优化整改，下面只对相关主流程做大致说明。
...
#### 4.2 PBFT不违背FLP定理
仔细观察，会发现PBFT的设计包含了超时机制，相当于一种同步通信模型。在超时机制下，PBFT能够保证安全性和活性。另外，在PBFT中，消息发送者是经过认证的。
从这一点来看，其实在PBFT的假设中已经不存在发送伪造消息的恶意节点了，最多是节点不响应。
...
## 共识算法总结
本文档共近3万字，主要讨论了传统分布式一致性算法和区块链场景使用的共识算法。首先从共识领域的拜占庭将军问题作为引子，逐步描述了各类一致性协议或共识算法。
