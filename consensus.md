...
一个可终止的共识算法。因此，Paxos也作出了一定的牺牲，它牺牲「部分」的活性从而保证了系统的安全性。即在系统处于异步状态时暂停共识推进，但只要有半数
以上的节点恢复至同步状态，就可以推进共识，完成终止。总的来说，Paxos具有如下特点：
...
2. N<sub>h</sub>，V<sub>h</sub>：本节点响应过的最大提案号
...
2. 每个Acceptor在收到一条Prepare(N)请求后，首先判断其中的提案号N是否大于本节点响应过的N<sub>h</sub>:
   ...
   2. 否则，发送一个Promise(N<sub>a</sub>，V<sub>a</sub>)承诺消息给Proposer，并更新自己的N<sub>h</sub>=N，V<sub>h</sub>=V表示承诺本节点不会再接受任何提案号
