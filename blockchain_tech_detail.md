...
- 重入攻击，是由于外部函数对合约的send()或者call.value()方法进行了递归调用造成的漏洞；
- 访问控制攻击：是由于合约未设置函数的访问权限而造成的漏洞；
- 整数溢出：是由于在这个语言中，要计算的数如果超过了EVM能处理的数字范围（0\~2<sup>256</sup>-1）就会重新从0或2<sup>256</sup>-1开始计数；
- 未检查返回值：是由于合约使用了send()或call.value()函数，却没有检查函数是否执行成功就进行下一步操作导致的漏洞；
- 拒绝服务：是由于攻击者恶意回退函数执行失败，使得原合约无法进行下一步操作而造成的漏洞；
